...
const controller = {
  ...
  getAllBlogsByUser: wrap ( async (req: Request, res: Response): Promise<Response | void> => {
     const { skip ,limit } = req.query
      ...
      const { blogs, count, unreadCount } = await blogService.getAllBlogsByUser( 
        userId ,
        Number(skip),
        Number(limit)
      )
  ...
  getAllBlogs: wrap ( async (req: Request, res: Response): Promise<Response | void> => {
     const { skip = 0, limit =0 } = req.query
      ...
      const { count, unreadCount, all } = await blogService.getAllBlogs( 
        {} ,
        Number(skip),
        Number(limit)
      )
  ...
  universalSearch: wrap ( async (req: Request, res: Response): Promise<Response | void> =>{ 
      ...
      }, "_id image title description", skip as number, limit as number)
  ...
  getBlogsFromUser: wrap ( async (req: Request, res: Response): Promise<Response | void> => {
       ...
       const BlogData = await Promise.all(user?.blogs.map((ele: customInterface) => {
